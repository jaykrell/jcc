...
static uint64_t jhash_mod(uint64_t a, uint64_t b)
{
	switch (b)
	{
#define JHASH_PRIME(x) case x: return a % x;
		JHASH_PRIMES
#undef JHASH_PRIME
	default: return a % b;
	}
...
void jhash_delete(jhash_t* hash)
{
	/* TODO */
}
...
static int jhash_insert_keyvalue(jhash_t *hash, jhash_lookup_t *lookup, jhash_keyvalue_t *keyvalue) {
...
int jhash_insert(jhash_t *hash, jhash_lookup_t *lookup) {
  ...
  if (err = jhash_new_buckets(hash, jhash_decide_bucket_count(2 * (hash->element_count + 1)))) {
	free(keyvalue);
    return err;
  }
